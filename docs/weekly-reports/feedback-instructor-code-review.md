...
|                            | ⚠️      | 100% cyclomatic coverage for non-GUI and non-enum code.                                                                                                                         | There is some missing coverage in TurnTracker that deserves test cases. For instance, the throw in setNumTotalPlayers and setDirection |
...
### Other comments
Overall, good job!!! One easy structure refactoring your team can do at the end for better application of the architecture: 
- everything in ui.model package should go in domain
- CardServices class should be in package domain
- Main.java in domain should be deleted
- Main.java under src/main/java should be deleted too

## Week 7 Code Review
I have read every line of production code currently in the main branch.
I didn't find any violations in terms of code quality according to letter grade A standards.
However, I did notice there are methods like TurnTracker#changeCurrentDirection
that is not being used by any method. This means there is no test for it.
This is a violation of TDD.
...
One minor thing: the input validation code (the 3 if statements) in `public List<Card> peekTop(int n)` method of `Deck` can be encapsulated into a private method
to comply with the "each function does one thing" principle better.
