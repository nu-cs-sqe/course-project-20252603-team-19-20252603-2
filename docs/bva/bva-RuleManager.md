...
| TC7         | actor holds 1 `CAT_CARDS` + 1 `FERAL CAT`, `requireCatPair(actor, CAT_CARDS)` | returns normally                                                                                | no           |
| TC8         | actor holds 1 `ATTACK` + 1 `FERAL CAT`, `requireCatPair(actor, ATTACK)`       | throws `IllegalStateException` with message `"rule.catPair.needTwo"`                            | no           |
| TC9         | actor holds 1 `FERAL CAT` + 1 `FERAL CAT`, `requireCatPair(actor, FERAL_CAT)` | throws `IllegalStateException` with message `"rule.catPair.rule.catPair.feralCannotBeBaseType"` | no           | 
